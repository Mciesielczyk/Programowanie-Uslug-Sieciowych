...
### Scenariusz 4: Weryfikacja integralności (wykrycie błędnego HMAC)

Scenariusz ten demonstruje działanie mechanizmu HMAC-SHA256 w przypadku próby manipulacji treścią pakietu przez osoby trzecie lub wystąpienia błędów w transmisji.

```
 [Sesja w toku - trwa tura Gracza A]

→ MOVE (oryginalny zamiar gracza)
  payload:  { "row": 1, "col": 1 }
  hmac:     "a3f5c2..." (obliczony na podstawie "row": 1, "col": 1)

 [Atakujący przechwytuje pakiet i zmienia wartość pola 'row' z 1 na 0]

→ MOVE (zmodyfikowany przez atakującego)
  payload:  { "row": 0, "col": 1 }
  hmac:     "a3f5c2..." (podpis pozostaje stary, wygenerowany dla pierwotnych danych)

 [DZIAŁANIE SERWERA]
 1. Serwer odbiera pakiet i wyodrębnia ciało JSON oraz otrzymany podpis HMAC.
 2. Serwer wylicza własny skrót HMAC na podstawie odebranego payloadu ("row": 0) 
    przy użyciu współdzielonego klucza SECRET_KEY.
 3. Serwer porównuje wyliczony skrót z tym otrzymanym w pakiecie ("a3f5c2...").
 4. Ponieważ skróty się nie zgadzają, serwer identyfikuje naruszenie integralności danych.

← ERROR
  payload: { "code": 401, "msg": "Błąd integralności danych (HMAC mismatch)" }

 [Serwer natychmiast zamyka połączenie z klientem w celu ochrony sesji]
 [Drugi gracz otrzymuje powiadomienie WIN z powodu rozłączenia przeciwnika]
